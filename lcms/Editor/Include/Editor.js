...
function document.onreadystatechange(){
	...
	if (!config.License){
		try{
			eWebEditor_License.innerHTML = "<span style=\"font-family:'Arial';\">&copy; <a href='http://www.gomye.net' target='_blank'><font color=#000000>GOMYE.NET</font></a></span>";
		}
		catch(e){
		}
	}
...
if (element.YUSERONCLICK) eval(element.YUSERONCLICK + ((BrowserInfo.MajorVer>7) ? "onclick(event)" : "anonymous()"));
...
// 改变模式：代码、编辑、文本、预览
function setMode(NewMode,msgFlag){
...
function insert(what) {
	...
	case "small":		// 字体变小
		...
		break;
	case "favorite":	//插入加入收藏
		insertHTML('<a href="javascript:void(0);" onClick="window.external.AddFavorite(document.location.href,document.title);">加入收藏</a>');
		break;
	default:
		alert("错误参数调用！");
		break;
