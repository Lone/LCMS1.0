...
Sub DoUpload_Class()
	...
	Dim UserTrueIP, Admin_Name, tSQL, FileSize
	
	UserTrueIP = Request.ServerVariables("HTTP_X_FORWARDED_FOR")
	If UserTrueIP="" Then UserTrueIP = Request.ServerVariables("REMOTE_ADDR")
	
	Admin_Name = Request.Cookies("LoneCMS2.0")("Admin_Name")
	If Admin_Name="" Then Admin_Name = Request.Cookies("Member.Name")
	FileSize = oFile.FileSize

	tSQL = "Insert Into Upload_Files (FileName, FileSize, URL, UploadUser, IP) Values (" &_
		"'" & sOriginalFileName & "', " &_
		"'" & FileSize & "', " &_
		"'" & sUploadDir & sSaveFileName & "', " &_
		"'" & Admin_Name & "', " &_
		"'" & UserTrueIP & "')"
	oConn.Execute(tSQL)
...
Function GetRndFileName(sExt)
	Dim sRnd, tempFileName, NowTime
	Randomize
	tempFileName = ""
	sRnd = Int(900 * Rnd) + 100
	NowTime = Now()
	If nAutoDir=0 Then tempFileName = tempFileName & Year(NowTime) & Month(NowTime)	& Day(NowTime)
	If nAutoDir=1 Then tempFileName = tempFileName & Month(NowTime)	& Day(NowTime)
	If nAutoDir=2 Then tempFileName = tempFileName & Day(NowTime)
		
	GetRndFileName = tempFileName & Hour(NowTime) & Minute(NowTime) & Second(NowTime) & sRnd & "." & sExt
End Function
...
Sub InitUpload()
	...
	If Not oRs.Eof Then
		...
		sUploadDir = AutoCreateDir(Int(nAutoDir))
...
Function RootPath2DomainPath(url)
	Dim sHost, sPort
	sHost = Split(Request.ServerVariables("SERVER_PROTOCOL"), "/")(0) & "://" & Request.ServerVariables("SERVER_NAME")
...
Function SaveRemoteFile(s_LocalFileName, s_RemoteFileUrl)
	...
	Admin_Name = Request.Cookies("LoneCMS2.0")("Admin_Name")
	If Admin_Name="" Then Admin_Name = Request.Cookies("Member.Name")
	FileSize = LenB(GetRemoteData)

	tSQL = "Insert Into Upload_Files (FileName, FileSize, URL, UploadUser, IP) Values (" &_
		"'" & s_RemoteFileUrl & "', " &_
		"'" & FileSize & "', " &_
		"'" & sUploadDir & s_LocalFileName & "', " &_
		"'" & Admin_Name & "', " &_
		"'" & UserTrueIP & "')"
	oConn.Execute(tSQL)
...
Function AutoCreateDir(n)
	AutoCreateDir = sUploadDir
	If n=0 Then Exit Function
	Dim fs, tDir
	Set fs = Server.CreateObject("Scripting.FileSystemObject")
	Select Case n
	Case 1:
		tDir = sUploadDir & CStr(Year(Date())) & "/"
		If Not fs.FolderExists(Server.MapPath(tDir)) Then
			fs.CreateFolder(Server.MapPath(tDir))
		End If
	Case 2:
		tDir = sUploadDir & CStr(Year(Date())) & "-" & CStr(Month(Date())) & "/"
		If Not fs.FolderExists(Server.MapPath(tDir)) Then
			fs.CreateFolder(Server.MapPath(tDir))
		End If
	Case 3:
		tDir = sUploadDir & CStr(Year(Date())) & "-" & CStr(Month(Date())) & "-" & CStr(Day(Date())) & "/"
		If Not fs.FolderExists(Server.MapPath(tDir)) Then
			fs.CreateFolder(Server.MapPath(tDir))
		End If
	End Select
