...
function set_cele_date(year,month,day)
{
   ...
   while(month-1==nd.getMonth())
    { j=(nd.getDay() +1);
      ...
      if(day && day!=""){
	       if ((nd.getDate()==day)&&(cele_date_month.value==month)&&(cele_date_year.value==year)){
...
function show_cele_date(eP,d_start,d_end,t_object,cur_date)
{
...
event.cancelBubble=true;
window.cele_date.style.left = event.clientX-event.offsetX+sL-5;  
window.cele_date.style.top = (event.clientY+eH+sT+dH-15)+'px';
if (window.cele_date.style.left+window.cele_date.clientWidth>document.body.clientWidth) window.cele_date.style.left+=eP.offsetWidth-window.cele_date.clientWidth;
if (d_start!=""){
    if (d_start=="today"){
