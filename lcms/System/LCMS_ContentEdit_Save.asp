...
Sub chkPostData()
	...
	End If
	Content_Id			= RequestForm("Content_Id")
...
Sub SaveContent()
	...
	'If Content_Locked=1 Then
		Content_URL 			= Rs("Content_URL")		
		Rs("Content_URL") 		= ""
		If Content_URL<>"" Then
			If Right(Content_URL, 1)="/" Then Content_URL = Content_URL & DEFAULT_FILENAME
			MapPath = Server.MapPath(Content_URL)
			Set FS = Server.CreateObject(Lone_FSO)
			If FS.FileExists(MapPath) Then
				FS.DeleteFile MapPath, True
				DeleteBlankFolder Left(Content_URL, InstrRev(Content_URL, "/"))
			End If
			Set FS = Nothing
		End If
	'End If
	...
	Lone.Execute("Update LCMS_Content Set Content_Orders = Content_Orders + 1")
	sLog = Lone.Admin_Name & "修改:" & Content_Title
	Lone.AddToLog(sLog)
End Sub 
