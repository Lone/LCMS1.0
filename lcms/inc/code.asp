...
Sub Com_CreatValidCode(pSN)
	Const codeLen = 5						'验证码位数
	Const cOdds = 2 						'杂点出现的机率
	Const dbtTimes = 2					'干扰次数（安全考虑，最好不要小于3）

	Const cAmount = 10 					'字库数量
	Const cCode = "0123456789"	'字库对应的字符
	Const UnitWidth = 16				'字宽(要为4的倍数)
	Const UnitHeight = 13				'字高
	Const DotsLimit = 1				'每次删除有效点的上限(避免无法人为识别)
	Const tryCount = 3					'避免删除有效点超过上限的尝试次数限制
	...
	' 颜色的数据(字符，背景)
	Dim vColorData(1)
	vColorData(0) = ChrB(0) & ChrB(0) & ChrB(200)  ' 蓝0，绿0，红0（黑色）
	vColorData(1) = ChrB(200) & ChrB(208) & ChrB(212) ' 蓝250，绿236，红211（浅蓝色）
